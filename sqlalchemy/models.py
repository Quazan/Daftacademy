# coding: utf-8
import json
from sqlalchemy import ARRAY, Column, DateTime, ForeignKey, Integer, Numeric, SmallInteger, String, Table, Text, text
...
class FilmActor(Base):
    ...
    film_id = Column(SmallInteger, primary_key=True, nullable=False, index=True)
    last_update = Column(TIMESTAMP(precision=6), nullable=False, server_default=text("now()"))


t_film_category = Table(
    'film_category', metadata,
    Column('film_id', SmallInteger, nullable=False),
    Column('category_id', SmallInteger, nullable=False),
    Column('last_update', TIMESTAMP(precision=6), nullable=False, server_default=text("now()"))
)
